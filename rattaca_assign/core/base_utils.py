...
def second_trait_density(
    ...
    outdir=None):
    """
    Produce an assignment density plot: trait z-score density for a secondary trait
    (not the primary assignment trait). Plotted assignments are colored by assignment
    groups for the assignment trait. 
    ...
    assignment_trait : str
        The name of the trait used to assign groups. Must be the column header for the 
        trait predictions to be plotted.

    plot_trait : str
        The trait of interest, to be plotted. Predictions for this trait will be colored 
        by assignment groups for assignment_trait.
    ...
    # columns to process for plotting
    zscore_col = f'{plot_trait}_zscore'
    group_col = f'{assignment_trait}_group'
    ...
    # extract predictions & group designations for the trait of interest
    trait_cols = [col for col in preds_df.columns if col.startswith(assignment_trait) or col.startswith(plot_trait) or col == 'rfid']
    trait_preds = preds_df[trait_cols].copy()

    # get assignment group ('high/low') designations for all samples
    trait_preds[group_col] = trait_groups(preds=preds_df, trait=assignment_trait, n_groups=n_groups)
    ...
    if len(trait_df) > 0:
        assigned_zscores = trait_df[zscore_col].dropna().values
        high_zscores = high_df[zscore_col].dropna().values
        low_zscores = low_df[zscore_col].dropna().values
        ax.vlines(high_zscores, 0, -0.018, color=sns.color_palette('inferno', n_colors=10)[7], alpha=1.0, linewidth=1.2)
        ax.vlines(low_zscores, 0, -0.018, color=sns.color_palette('inferno', n_colors=10)[2], alpha=1.0, linewidth=1.2)
    ...
    if len(low_df) > 0:
        low_scatter = ax.scatter(
            ...
            color = sns.color_palette('inferno', n_colors=10)[2],
            zorder = 3,
            label = f'Low {assignment_trait}'
        )
        legend_handles.append(low_scatter)
    ...
    if len(high_df) > 0:
        high_scatter = ax.scatter(
            ...
            color = sns.color_palette('inferno', n_colors=10)[7], 
            zorder = 3,
            label = f'High {assignment_trait}'
        )
        legend_handles.append(high_scatter)
    
    # add medium group if n_groups = 3
    if n_groups == 3 and len(med_df) > 0:
        med_scatter = ax.scatter(
            x = med_df[zscore_col].values,
            y = np.zeros_like(med_df[zscore_col].values),
            s = 40,
            color = sns.color_palette('inferno', n_colors=10)[5],
            zorder = 3,
            label = f'Med {assignment_trait}'
        )
    ...
    # add legend
    if legend_handles:
        ax.legend(handles=legend_handles, loc='best', frameon=True, fancybox=False, edgecolor='black')
    ...
    if gen is not None:
        ax.set_title(f'RATTACA gen {gen}\n{trait_name} assignments\n(grouped by {assignment_trait})', pad=15)
    else:
        ax.set_title(f'{trait_name} assignments', pad=15)
